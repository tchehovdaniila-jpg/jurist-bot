...
def create_pdf(text, filename="contract.pdf"):
    """Создаёт PDF с русским текстом через weasyprint"""
    try:
        # Оборачиваем текст в HTML с указанием кодировки
...
# Шаблоны договоров (русские, как ты хотел)
CONTRACTS = {
    'rent': {
        'name': 'Аренда квартиры',
        'questions': [
            '📍 Город:', '📅 Дата:', '👤 ФИО Арендодателя:', '📄 Паспорт Арендодателя:',
            '🏠 Адрес регистрации Арендодателя:', '👤 ФИО Арендатора:', '📄 Паспорт Арендатора:',
            '🏠 Адрес регистрации Арендатора:', '🏢 Адрес квартиры:', '💰 Сумма аренды (цифрами):',
            '✍️ Сумма прописью:', '⏱️ Срок аренды:'
        ],
...
def main():
    ...
    logger.info("✅ Бот с PDF (WeasyPrint) запущен")
